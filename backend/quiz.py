...
# Load ML model
model_data = joblib.load("models/mental_health_model.joblib")
...
def predict(data: Answers):
    df = pd.DataFrame([data.answers], columns=[f"Q{i+1}" for i in range(10)])
    ...
    confidence_scores = {label: float(pred_proba[i]) for i, label in enumerate(label_encoder.classes_)}
